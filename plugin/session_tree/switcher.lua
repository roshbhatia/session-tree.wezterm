...
local ui_actions = require("session_tree.actions")
local ui_badges = require("session_tree.adapters").badges
local ui_panes = require("session_tree.adapters").panes
...
function M.setup(config, wm, ctx)
  ...
  local session_tree_choices = require("session_tree.tree_rows").new(ctx)
  ...
  config.key_tables.session_tree_selector = {
    {
      key = "Enter",
      mods = "NONE",
  ...
  for _, spec in ipairs(SESSION_TREE_ACTIONS) do
    config.key_tables.session_tree_selector[#config.key_tables.session_tree_selector + 1] = {
      key = spec.key,
      mods = "NONE",
      action = wezterm.action_callback(function(win, pane)
        if spec.id:sub(1, 9) == "provider:" then
          launcher.open(win, pane, spec.key, function()
            if win:active_key_table() ~= "session_tree_selector" then
              return false
            end
            win:perform_action(wezterm.action.PopKeyTable, pane)
          end)
  ...
  open_session_tree = function(win, pane, filter, notice)
    ...
    options.action = wezterm.action_callback(function(inner_win, inner_pane, id, _label)
      ...
      pcall(function()
        if inner_win:active_key_table() == "session_tree_selector" then
    ...
    win:perform_action(wezterm.action.ActivateKeyTable({ name = "session_tree_selector", one_shot = false }), pane)
    win:perform_action(wezterm.action.InputSelector(options), pane)
    launcher.prefetch()
  end
  ...
  if wm then
    ...
    wm.session_state_dir = options.get().workspace_state_dir or wm.session_state_dir
  ...
  local binding = options.get().binding
  if binding then
    ...
    config.keys[#config.keys + 1] = {
      ...
      mods = binding.mods,
      action = wezterm.action_callback(function(win, pane)
        if options.get().locked and options.get().locked() then
          win:perform_action({ SendKey = { key = binding.key, mods = binding.mods } }, pane)
          return
        end
  ...
  wezterm.on("augment-command-palette", function(_window, _pane)
    local entries = {
      ...
      },
      {
        ...
        action = wezterm.action_callback(function(win, pane)
          local names = ui_sessions.active_names()
          table.sort(names, function(a, b)
            return (ui_sessions.last_active(a) or 0) > (ui_sessions.last_active(b) or 0)
          end)
          for _, name in ipairs(names) do
            if name ~= win:active_workspace() then
              ui_actions.switch_to_workspace(win, pane, name)
              return
            end
          end
        end),
